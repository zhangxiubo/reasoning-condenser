...
A run of `LOOP_BREAKER_DECAY_AFTER_CLEAN` consecutive clean requests lowers the level by one; a model that stays clean long enough walks back to zero and is forgotten. At the default of `2` or higher, escalation is faster than decay: a single unrelated call between stalls still climbs rather than reversing it. At `1`, each clean request cancels the increment from the stall before it, so alternating stall and clean makes no net progress toward the hard stop, wherever the level already stands.
