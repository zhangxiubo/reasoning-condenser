...
Escalation is faster than decay by design. A model that recovers walks its level back to zero. A model that emits one unrelated call between stalls still climbs, because a single clean request never cancels a stall.
